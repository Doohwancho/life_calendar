...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  eventBus.on("dataChanged", (payload) => {
    ...
    // // 연간 달력 업데이트
    // if (
    //   payload?.source === "updateCurrentWeeklyViewStartDate" ||
    //   payload?.source === "updateCurrentDisplayYear" ||
    //   payload?.source === "fileLoad" ||
    //   payload?.source === "addEvent" ||
    //   payload?.source === "deleteEvent" ||
    //   payload?.source === "updateEventDates" ||
    //   payload?.source === "deleteLabelAndEvents" ||
    //   payload?.source === "updateLabelName" ||
    //   payload?.source === "addLabel"
    // ) {
    //   // 위 source들은 연간 달력의 전체 구조 또는 주요 내용 변경을 유발할 수 있음
    //   if (typeof renderYearlyCalendar === "function") {
    //     console.log(
    //       "APP.JS: Re-rendering full Yearly Calendar for year:",
  ...
  if (saveLabelBtn)
    ...
    async function handleSaveData() { 
      ...
      //     `yearly/${currentState.currentDisplayYear}.json`, 
      //     data.getAllDataForSave() 
      // );
      // const settingsToSave = {
      //     version: currentState.version || "3.0", 
      //     colorPalette: currentState.colorPalette || [], 
      //     lastOpenedYear: currentState.currentDisplayYear 
      // };
      // dirtyFileService.markFileAsDirty('settings.json', settingsToSave);
      
      // 3. Global Save (ZIP 다운로드) 호출
      // --- [Phase 6] 수정: dirtyFileService 네임스페이스를 통해 함수 호출 ---
      await dirtyFileService.triggerGlobalSave(); 
    }
  ...
  function handleLoadData(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // 불러온 데이터(YYYY.json의 내용)를 dataManager로 전달
        data.loadAllData(loadedData);
        // loadAllData가 dataChanged 이벤트를 발생시켜 UI가 자동으로 업데이트됨
