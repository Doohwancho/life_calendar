...
function handleYearlyCellDoubleClick(e) {
  const cell = e.currentTarget; // 이벤트 리스너가 연결된 .day-cell-yearly 요소
  const dateStr = cell.dataset.date;

  if (dateStr) {
      console.log(`Yearly cell double-clicked: ${dateStr}. Redirecting...`);
      // Daily Page의 파일 이름을 daily_view.html로 가정합니다.
      // URL 파라미터로 날짜를 전달합니다.
      window.location.href = `daily_view/index.html?date=${dateStr}`;
  }
}
