...
const ROUTINES_LOCAL_STORAGE_KEY = 'timeLedgerRoutines_v3'; // Version bump
let routines = [];
...
export function initRoutinesApp(containerSelector, initialData = null, dataChangedCallback = () => {}) {
    ...
    if (initialData && Array.isArray(initialData)) { 
        ...
        const stored = (typeof localStorage !== 'undefined') ? localStorage.getItem(ROUTINES_LOCAL_STORAGE_KEY) : null;
        if (stored) { 
            try { routines = JSON.parse(stored); } 
            catch(e) { 
                console.error("Error parsing routines from local storage", e); routines = []; 
            } 
        }
        // 로컬 스토리지에도 없고, 초기 데이터도 없을 때 기본값 설정
        if (!routines || routines.length === 0) { 
            routines = [ 
                { id: generateId(), timeOfDay: "아침", activity: "명상", memo: "10분 집중" },
                { id: generateId(), timeOfDay: "저녁", activity: "하루 계획 검토", memo: "다음 날 준비" }
            ]; 
        }
    }
    saveRoutinesToLocalBackup(); // 현재 상태를 로컬 백업
    renderRoutinesTableInternal();
}
export function setRoutinesDataAndRender(newData) { routines = Array.isArray(newData) ? newData : []; saveRoutinesToLocalBackup(); renderRoutinesTableInternal(); }
export function getRoutinesData() { return JSON.parse(JSON.stringify(routines)); }