...
function createTodoElement(item, index) {
    ...
    li.addEventListener('dragstart', (e) => {
        ...
        // Optional: e.dataTransfer.setData('text/plain', item.id);
    });
...
export function initTodoApp(containerSelector, initialData = null, dataChangedCallback = () => {}) {
    ...
    if (initialData && Array.isArray(initialData)) {
        todos = initialData.map(todo => ({ ...todo, id: todo.id || generateId() })); // Ensure IDs
    } else {
        const storedTodos = (typeof localStorage !== 'undefined') ? localStorage.getItem(TODO_LOCAL_STORAGE_KEY) : null;
        if (storedTodos) {
            try {
                todos = JSON.parse(storedTodos).map(todo => ({ ...todo, id: todo.id || generateId() })); // Ensure IDs
            } catch (e) {
                console.error("Error parsing todos from localStorage:", e);
                todos = []; // Default to empty if parsing fails
            }
        }
        if (!todos || todos.length === 0) { // Default tasks if no stored data or empty
            todos = [
                { id: generateId(), text: 'Sample Task 1 (Drag me!)', importance: 1, time: 15, completed: false },
                { id: generateId(), text: 'Sample Task 2', importance: 2, time: 45, completed: true },
                { id: generateId(), text: 'Another draggable task', importance: 0, time: 5, completed: false },
            ];
        }
    }
    saveTodosToLocalBackup(); // Call to ensure onTodoDataChangeCallback is fired initially
    renderTodoListInternal();
}
