...
function getStorageKey(fileIdentifier) {
  ...
  if (fileIdentifier === "mandal-art.json") {
    return "dirty_mandal_art";
  }
  ...
  if (parts.length === 2) {
    ...
    } else if (filename.startsWith(year)) {
      // "2025-05"
...
function getOriginalFilename(storageKey) {
  ...
  if (storageKey === "dirty_mandal_art") {
    return "mandal-art.json";
  }
...
export function markFileAsDirty(fileIdentifier, dataObject) {
  ...
  try {
    ...
    let dirtyKeys = JSON.parse(
      localStorage.getItem(DIRTY_KEYS_LIST_KEY) || "[]"
    );
    ...
    console.log(
      `[DirtyFileService - markFileAsDirty] Key: ${storageKey}, File: ${fileIdentifier}`
    );
  } catch (e) {
    console.error(
      `Error marking file dirty in localStorage for ${fileIdentifier}:`,
      e
    );
  }
}
...
export function getDirtyFileData(fileIdentifier) {
  ...
  if (dataString) {
    try {
      ...
      console.error(
        `Error parsing dirty data from localStorage for ${fileIdentifier} (key: ${storageKey}):`,
        e
      );
      return null;
    }
...
export async function triggerFullYearSave(year, filesToSave) {
  ...
  try {
    ...
    console.error("Error generating full year backup ZIP:", e);
