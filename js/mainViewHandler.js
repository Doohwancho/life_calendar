...
import {
  ...
  onDataChanged as onEisenhowerDataChanged,
} from "./eisenhowerMatrix.js";
  ...
  labels.forEach((label) => {
    ...
    const labelContextMenuHandler = (e) => {
      ...
      unscheduleItem.onclick = () => {
        if (
          confirm(
            `'${label.name}' 프로젝트의 모든 캘린더 일정을 제거하시겠습니까? (프로젝트 목록에서는 사라지지 않습니다.)`
          )
        ) {
      ...
      permanentDeleteItem.onclick = () => {
        if (
          confirm(
            `'${label.name}' 프로젝트와 내부의 모든 할 일, 그리고 캘린더의 모든 관련 일정이 영구적으로 삭제됩니다.\n\n정말 삭제하시겠습니까?`
          )
        ) {
      ...
      setTimeout(() => {
        window.addEventListener("click", handleOutsideContextMenuClick, {
          once: true,
        });
      }, 0);
    };
    ...
    activeEventListeners.push({
      element: labelEl,
      type: "contextmenu",
      handler: labelContextMenuHandler,
    });
...
async function handleSaveCurrentYear() {
  ...
  try {
    ...
    console.log(
      `[MainViewHandler] Data for year ${currentYear} saved to ZIP. LocalStorage dirty data for this year is intentionally kept.`
    );
  } catch (e) {
...
function handleLoadYearlyData(event) {
  ...
  if (!yearMatch || !yearMatch[1]) {
    ...
    alert(
      "올바른 형식의 연간 백업 파일명이 아닙니다. 파일 이름이 'backup_YYYY'로 시작해야 합니다. (예: backup_2025_....zip)"
    );
    event.target.value = "";
    return;
  }
        ...
        if (!zipEntry.dir) {
          ...
          const promise = zipEntry.async("string").then((content) => {
            try {
              ...
              console.error(
                `Error parsing JSON from ${relativePath}:`,
                parseError
              );
            }
          });
...
// --- Main View 초기화 함수 ---
export async function initMainCalendarView(
  dataModule,
  eventBusModule,
  params,
  query
  ...
  const dataChangedHandler = (payload) => {
    console.log(
      `[MainViewHandler] eventBus 'dataChanged' received. Source: ${payload?.source}`
    );
    ...
    renderBacklog();
    onEisenhowerDataChanged(); // 아이젠하워 매트릭스 업데이트
  };
  ...
  if (showAddTodoFormBtn && addTodoFormContainer && newTodoTextInput) {
    ...
    activeEventListeners.push({
      element: showAddTodoFormBtn,
      type: "click",
      handler: showAddTodoFormClickHandler,
    });
  }
  ...
  if (saveNewTodoBtn) {
    ...
    activeEventListeners.push({
      element: saveNewTodoBtn,
      type: "click",
      handler: handleAddNewTodo,
    });
  }
  ...
  if (
    ...
    activeEventListeners.push({
      element: cancelNewTodoBtn,
      type: "click",
      handler: cancelNewTodoClickHandler,
    });
  }
  ...
  if (backlogListContainer) {
    ...
    activeEventListeners.push({
      element: backlogListContainer,
      type: "dragover",
      handler: handleBacklogDragOver,
    });
  ...
  if (yearlyGoalInput) {
    ...
    activeEventListeners.push({
      element: yearlyGoalInput,
      type: "input",
      handler: yearlyGoalInputHandler,
    });
    ...
    activeEventListeners.push({
      element: yearlyGoalInput,
      type: "keydown",
      handler: yearlyGoalKeydownHandler,
    });
  }
  ...
  if (labelsContainer) {
    const labelsDragOverHandler = (e) => {
      e.preventDefault();
      const draggingEl = labelsContainer.querySelector(
    ...
    const labelsDropHandler = (e) => {
      ...
      const newLabelOrderIds = Array.from(
        labelsContainer.querySelectorAll(".mv-label-item")
  ...
  if (saveLabelBtn && addLabelModal && labelNameInput && labelColorInput) {
    ...
    activeEventListeners.push({
      element: saveLabelBtn,
      type: "click",
      handler: performSaveLabel,
    });
    ...
    activeEventListeners.push({
      element: labelNameInput,
      type: "keydown",
      handler: labelNameInputKeydownHandler,
    });
    ...
    activeEventListeners.push({
      element: labelColorInput,
      type: "keydown",
      handler: labelColorInputKeydownHandler,
    });
  }
  ...
  if (loadDataBtn && fileInput) {
    ...
    activeEventListeners.push({
      element: loadDataBtn,
      type: "click",
      handler: loadDataBtnClickHandler,
    });
  }
  if (fileInput) {
  ...
  console.log(
    "[MainViewHandler] Initial data is pre-loaded. Triggering initial render."
  );
    ...
    if (target) {
      if (listener.isEventBus) {
        ...
        if (typeof target.off === "function") {
          ...
          console.log(
            `[MainViewHandler] EventBus listener for '${listener.type}' removed.`
          );
        }
      } else {
