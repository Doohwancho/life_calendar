...
import {
  initMainCalendarView,
  cleanupMainCalendarView,
} from "./mainViewHandler.js";
import {
  initDailyDetailView,
  cleanupDailyDetailView,
} from "./dailyViewHandler.js";
import {
  initMandalArtView,
  cleanupMandalArtView,
} from "./mandalArtViewHandler.js";
import {
  initJournalOverview,
  cleanupJournalOverview,
} from "./journalOverviewHandler.js";
...
const routes = {
  ...
  "/daily/:date": {
    // Daily View 경로 (예: #/daily/2025-05-30)
  ...
  "/journal-overview": {
    template: "views/journal-overview.html",
    init: initJournalOverview,
    cleanup: cleanupJournalOverview,
...
async function loadTemplate(templatePath) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Template load error: ${templatePath} (Status: ${response.status})`
      );
    }
...
async function handleRouteChange() {
  console.log(
    "[SPA Router] Route change detected. Current hash:",
    location.hash
  );
  ...
  if (routeConfig && viewContainer) {
    console.log(
      "[SPA Router] Matched route:",
      routeConfig.matchedPattern,
      "Params:",
      routeConfig.params
    );
    ...
    if (htmlContent) {
      ...
      if (typeof routeConfig.init === "function") {
        console.log(
          "[SPA Router] Initializing view for:",
          routeConfig.matchedPattern
        );
...
async function initializeApp() {
  ...
  window.addEventListener("load", () => {
    if (!location.hash || location.hash === "#") {
      ...
      const lastYear =
        dataManager.getState().lastOpenedYear || new Date().getFullYear();
