...
import {
  initTodoApp,
  getTodoData,
  setTodoDataAndRender as setAndRenderTodos,
} from "../daily_view/todo.js";
import {
  initRoutinesApp,
  getRoutinesData,
  setRoutinesDataAndRender as setAndRenderRoutines,
} from "../daily_view/routines.js";
import {
  initTimelines,
...
import {
  initDiaryApp,
  getDiaryData,
  setDiaryDataAndRender as setAndRenderDiary,
} from "../daily_view/diary.js";
...
function displayCurrentDate(dateStr) {
  ...
  if (dateDisplay && dateStr) {
    ...
    const now = new Date();
    dateDisplay.textContent = `${now.getFullYear()} / ${String(
      now.getMonth() + 1
...
function removeColorOptionContextMenu() {
  ...
  window.removeEventListener(
    "click",
    handleOutsideColorOptionContextMenuClick,
    true
  );
}
...
function showColorOptionContextMenu(event, colorData, colorOptionElement) {
  ...
  deleteItem.addEventListener("click", (e) => {
    ...
    savedColors = savedColors.filter(
      (c) => c.color.toLowerCase() !== colorData.color.toLowerCase()
    );
  ...
  setTimeout(() => {
    window.addEventListener("click", handleOutsideColorOptionContextMenuClick, {
      capture: true,
      once: true,
    });
  }, 0);
...
function renderColorOptionsUI() {
  ...
  savedColors.forEach(({ color, label }) => {
    ...
    colorOption.addEventListener("contextmenu", (e) => {
      showColorOptionContextMenu(
        e,
        { color, label: displayLabel },
        colorOption
      );
    });
...
function initializeColorPicker() {
  ...
  if (addColorBtn && !addColorBtn.dataset.listenerAttached) {
    ...
    activeEventListeners.push({
      element: addColorBtn,
      type: "click",
      handler: toggleColorPopupHandler,
    });
  }
  ...
  if (
    ...
    colorInput &&
    colorLabelInput &&
    !saveColorBtn.dataset.listenerAttached
  ) {
    const handleSaveNewColorAction = () => {
      ...
      if (
        !savedColors.some(
          (c) => c.color.toLowerCase() === newSelectedColor.toLowerCase()
        )
      ) {
    ...
    activeEventListeners.push({
      element: saveColorBtn,
      type: "click",
      handler: handleSaveNewColorAction,
    });
    ...
    if (!colorLabelInput.dataset.keydownListenerAttached) {
      ...
      activeEventListeners.push({
        element: colorLabelInput,
        type: "keydown",
        handler: colorLabelKeydownHandler,
      });
    }
    ...
    if (!colorInput.dataset.keydownListenerAttached) {
      ...
      activeEventListeners.push({
        element: colorInput,
        type: "keydown",
        handler: colorInputKeydownHandler,
      });
    }
  }
}
...
const outsideColorPopupClickHandler = (e) => {
  ...
  if (
    popup &&
    popup.classList.contains("dv-show") &&
...
function handleDataChange() {
  if (!localDataManager || !currentLoadedDate) {
    console.warn(
      "[DailyViewHandler] handleDataChange aborted: dataManager or currentLoadedDate missing."
    );
    return;
  }
  ...
  const monthDataObject = JSON.parse(
    JSON.stringify(
      localDataManager.getRawDailyDataForMonth(yearMonth) || {
        yearMonth,
        routines: [],
  ...
  const currentDaySpecificData = {
    todos:
      typeof getTodoData === "function"
        ? getTodoData().filter(
            (t) => !(t.injectedFromBacklogP1 && !t.completed)
          )
        : [],
    timeBlocks:
      typeof getTimelineBlockData === "function" ? getTimelineBlockData() : {},
    goalBlocks:
      typeof getTimelineGoalData === "function" ? getTimelineGoalData() : {},
    scheduledTimelineTasks:
      typeof getScheduledTasksData === "function"
        ? getScheduledTasksData()
        : [],
...
function setAllModuleData(
  daySpecificData = {},
  monthSharedData = {},
  activeProjects = []
) {
  // if (typeof setAndRenderProjectTodos === 'function') setAndRenderProjectTodos(activeProjects);
  if (typeof setAndRenderTodos === "function")
    setAndRenderTodos(daySpecificData.todos || []);
  if (typeof setAndRenderRoutines === "function")
    setAndRenderRoutines(monthSharedData.routines || []); // 월별 루틴 사용
  if (typeof setTimelineBlockDataAndRender === "function")
    setTimelineBlockDataAndRender(
      daySpecificData.timeBlocks || {},
      "timeGridDOM"
    );
  if (typeof setTimelineGoalDataAndRender === "function")
    setTimelineGoalDataAndRender(
      daySpecificData.goalBlocks || {},
      "goalTimeGridDOM"
    );
  if (typeof setScheduledTasksDataAndRender === "function")
    setScheduledTasksDataAndRender(
      daySpecificData.scheduledTimelineTasks || []
    );
  if (typeof setAndRenderDiary === "function")
    setAndRenderDiary(daySpecificData.diary || {});
}

// function updateElapsedTimeBlocksInternal() { /* ... */ }

// --- Daily View 초기화 함수 ---
export async function initDailyDetailView(
  dataModule,
  busModule,
  params,
  query
  ...
  console.log(
    `[DailyViewHandler] Daily View 초기화 시작 for date: ${params.date}`
  );
  ...
  if (
    localDataManager &&
    typeof localDataManager.getState === "function" &&
    typeof localDataManager.loadDataForYear === "function"
  ) {
    const dmState = localDataManager.getState();
    if (
      !dmState.yearlyData ||
      dmState.currentDisplayYear !== yearOfDailyView ||
      !localDataManager.getRawDailyDataForMonth(yearMonthKeyForDailyView)
    ) {
      console.log(
        `[DailyViewHandler] Data for year ${yearOfDailyView} not fully loaded. Forcing loadDataForYear.`
      );
      await localDataManager.loadDataForYear(yearOfDailyView);
    } else {
      console.log(
        `[DailyViewHandler] Data for year ${yearOfDailyView} is already loaded.`
      );
    }
  }

  const settings = localDataManager.getSettings(); // 최신 설정을 가져옴 (loadDataForYear 이후에 호출될 수 있도록)
  const monthDataObject = localDataManager.getRawDailyDataForMonth(
    yearMonthKeyForDailyView
  ) || {
  ...
  // --- Inject backlog P1 into today only ---
  const isToday = (() => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const [y, m, d] = currentLoadedDate.split("-").map(Number);
    const cur = new Date(y, m - 1, d);
    cur.setHours(0, 0, 0, 0);
    return cur.getTime() === today.getTime();
  })();

  let augmentedTodos = Array.isArray(dataForThisDate.todos)
    ? [...dataForThisDate.todos]
    : [];
  if (isToday) {
    try {
      const state = localDataManager.getState();
      const p1Backlogs = (state.backlogTodos || []).filter(
        (t) => t.priority === 1
      );
      const injected = p1Backlogs.map((t) => ({
        id: `inj_${t.id}`,
        text: t.text,
        importance: 1,
        time: 0,
        color: t.color,
        completed: false,
        injectedFromBacklogP1: true,
        originalBacklogId: t.id,
      }));
      // Order: injected P1 first, then existing
      augmentedTodos = [...injected, ...augmentedTodos];
    } catch (e) {
      /* noop */
    }
  }
  ...
  setAllModuleData(
    { ...dataForThisDate, todos: augmentedTodos },
    monthDataObject
  );
  ...
  if (typeof initProjectTodoApp === "function") {
    initProjectTodoApp(
      ...
      (changeInfo) => {
        // 콜백 함수 시그니처 변경: {type, payload}
        if (changeInfo && changeInfo.type === "TOGGLE_TODO") {
          localDataManager.updateProjectTodo(
            changeInfo.payload.projectId,
            changeInfo.payload.todoId,
            changeInfo.payload.completed
          );
  ...
  if (typeof initTodoApp === "function")
    initTodoApp("#todo-app-container", augmentedTodos || [], handleDataChange);
  if (typeof initRoutinesApp === "function")
    initRoutinesApp(
      "#routines-app-container",
      monthDataObject.routines || [],
      handleDataChange
    );
  if (typeof initDiaryApp === "function")
    initDiaryApp(
      "#diary-app-container",
      dataForThisDate.diary || {},
      handleDataChange
    );
  ...
  if (typeof initTimelines === "function") {
    ...
    if (typeof updatePassedTimeVisuals === "function") {
      const runLiveUpdates = () => {
        ...
        const [tlYear, tlMonth, tlDay] = timelineDateString
          .split("-")
          .map(Number);
        const timelineDateForComparison = new Date(tlYear, tlMonth - 1, tlDay); // JS 월은 0부터 시작

        const todayDateForComparison = new Date(
          now.getFullYear(),
          now.getMonth(),
          now.getDate()
        );
        ...
        if (
          timelineDateForComparison.getTime() ===
          todayDateForComparison.getTime()
        ) {
          // 케이스 1: 현재 보고 있는 뷰가 오늘 달력 날짜 (예: 6월 1일 뷰, 현재도 6월 1일)
          // 이 타임라인이 '오늘'의 유효한 타임라인인가? => 지금이 오전 6시 이후인가?
          isTimelineEffectivelyToday =
            currentActualHour >= ACTUAL_DAY_STARTS_AT_HOUR_CONST;
        } else if (
          timelineDateForComparison.getTime() < todayDateForComparison.getTime()
        ) {
          // 케이스 2: 현재 보고 있는 뷰가 어제 또는 그 이전 날짜 (예: 5월 31일 뷰, 현재는 6월 1일)
          const oneDayMilliseconds = 24 * 60 * 60 * 1000;
          const isImmediatelyPreviousDay =
            todayDateForComparison.getTime() -
              timelineDateForComparison.getTime() ===
            oneDayMilliseconds;
      ...
      const [loadedYear, loadedMonth, loadedDay] = currentLoadedDate
        .split("-")
        .map(Number);
      const currentLoadedDateObj = new Date(
        loadedYear,
        loadedMonth - 1,
        loadedDay
      );
      const todayObjForIntervalCheck = new Date(
        nowForIntervalCheck.getFullYear(),
        nowForIntervalCheck.getMonth(),
        nowForIntervalCheck.getDate()
      );
      const ACTUAL_DAY_STARTS_AT_HOUR_FOR_INTERVAL = 6;

      let shouldRunIntervalUpdates = false;
      if (
        currentLoadedDateObj.getTime() === todayObjForIntervalCheck.getTime()
      ) {
        // 오늘 뷰
        ...
        if (
          todayObjForIntervalCheck.getTime() -
            currentLoadedDateObj.getTime() ===
            oneDayMs &&
          nowForIntervalCheck.getHours() <
            ACTUAL_DAY_STARTS_AT_HOUR_FOR_INTERVAL
        ) {
          // 어제 뷰 + 지금 새벽
  ...
  if (typeof initYearProgress === "function")
    initYearProgress("#year-progress-header-container");
  if (typeof initDayProgress === "function")
    initDayProgress("#day-progress-bar-host");

  const existingDocClickListener = activeEventListeners.find(
    (l) =>
      l.target === document &&
      l.type === "click" &&
      l.handler === outsideColorPopupClickHandler
  );
  if (!existingDocClickListener) {
    ...
    activeEventListeners.push({
      target: document,
      type: "click",
      handler: outsideColorPopupClickHandler,
    });
  }
  ...
  async function handleSaveDataForCurrentDailyViewYear() {
    ...
    if (!localDataManager || !currentLoadedDate) {
      console.warn(
        "[DailyViewHandler] Cannot save, localDataManager or currentLoadedDate missing."
      );
      return;
    }
    const yearOfDailyView = parseInt(currentLoadedDate.substring(0, 4), 10);
    console.log(
      `[DailyViewHandler] Attempting to save all data for year: ${yearOfDailyView} (like main view's Cmd+S)`
    );
    ...
    if (typeof localDataManager.getCurrentYearDataForSave === "function") {
      ...
      console.error(
        "localDataManager.getCurrentYearDataForSave is not available."
      );
    ...
    if (!filesToSave || filesToSave.length === 0) {
      alert(`${yearOfDailyView}년에 저장할 데이터가 없습니다.`);
      return;
    }
    ...
    filesToSave.forEach((fileInfo) => {
      if (fileInfo.filenameInZip.includes("/")) {
        ...
        zip
          .folder(folderName)
          .file(fileName, JSON.stringify(fileInfo.data, null, 2));
      } else {
        zip.file(
          fileInfo.filenameInZip,
    ...
    try {
      ...
      alert(
        `${yearOfDailyView}년 데이터 백업 파일 생성 중 오류가 발생했습니다.`
      );
    }
  }
  ...
  const dailyViewKeydownHandler = (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key === "s") {
      e.preventDefault();
      console.log(
        "[DailyViewHandler] Ctrl+S pressed, initiating save for current daily view year."
      );
  ...
  activeEventListeners.push({
    target: document,
    type: "keydown",
    handler: dailyViewKeydownHandler,
  });
