...
import { generateId, getMondayOfWeek, formatDate } from "./uiUtils.js";
...
export function getState() {
  ...
  const currentMonthKey = `${state.view.currentDisplayYear}-${String(
...
export function loadDataForYear(year) {
  ...
  if (state.yearlyData && state.yearlyData.year === numericYear) {
    console.log(
      `DataManager: Year ${numericYear} is already loaded. Notifying UI to render.`
    );
    eventBus.dispatch("dataChanged", {
      source: "yearAlreadyLoaded",
      yearLoaded: numericYear,
    });
    return;
  }
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    const monthDataObject = dirtyFileService.getDirtyFileData(
      dailyFileIdentifier
    ) || {
  ...
  if (numericYear === today.getFullYear()) {
    ...
    state.view.currentWeeklyViewStartDate = getMondayOfWeek(
  ...
  if (state.mandalArt) {
    ...
    mandalArtToSave.mandalArts.forEach((mandal) => {
      ...
      mandal.cells.forEach((cell, index) => {
        const isModified =
          cell.content || cell.isCompleted || cell.isHighlighted || cell.color;
        if (isModified) {
          const modifiedProps = {};
...
export function loadYearFromBackup(year, filesData) {
  ...
  filesData.forEach((fileInfo) => {
    ...
    // 3. 월간 데이터 처리
      ...
      const yearMonthKey = filenameInZip
        .replace(`${numericYear}/`, "")
        .replace(".json", "");
...
export function clearAllDirtyFilesForYear(year) {
  ...
  for (let i = 1; i <= 12; i++) {
    const month = String(i).padStart(2, "0");
    dirtyFileService.clearDirtyFile(
      `${numericYear}/${numericYear}-${month}.json`
    );
  }
}
...
export function updateDailyData(yearMonth, newMonthDataObject) {
  ...
  if (String(state.view.currentDisplayYear) !== String(year)) {
    // 타입 일치시켜 비교
    console.warn(
      `Attempted to update daily data for ${yearMonth}, but current view is ${state.view.currentDisplayYear}. Update ignored.`
    );
    return;
  }
...
function getOrInitializeDayDataStructure(monthDataObject, dateStr) {
  ...
  if (!monthDataObject.dailyData[dateStr]) {
    ...
    if (monthDataObject.dailyData[dateStr].todos === undefined)
    ...
    if (monthDataObject.dailyData[dateStr].timeBlocks === undefined)
      monthDataObject.dailyData[dateStr].timeBlocks = {}; // 오타 수정
    if (monthDataObject.dailyData[dateStr].goalBlocks === undefined)
      monthDataObject.dailyData[dateStr].goalBlocks = {}; // 오타 수정
    if (monthDataObject.dailyData[dateStr].scheduledTimelineTasks === undefined)
      monthDataObject.dailyData[dateStr].scheduledTimelineTasks = [];
    if (monthDataObject.dailyData[dateStr].diary === undefined)
      monthDataObject.dailyData[dateStr].diary = {
        keep: "",
        problem: "",
        try: "",
      };
...
export function setCellMark(dateStr, markType) {
  ...
  if (!monthDataObject) {
    monthDataObject = {
      ...
      colorPalette:
        state.settings && state.settings.colorPalette
          ? [...state.settings.colorPalette]
          : [],
      dailyData: {},
    };
  ...
  const newMark =
    markType === "none" || markType === "" || markType === undefined
      ? null
      : markType;
  ...
  if (currentMark !== newMark) {
    ...
    console.log(
      `[DataManager setCellMark] Mark for ${dateStr} set to: ${newMark}`
    );
...
export function addLabel(project) {
  // label -> project
  if (!state.yearlyData)
    state.yearlyData = {
  ...
  dirtyFileService.markFileAsDirty(
    `${state.view.currentDisplayYear}/${state.view.currentDisplayYear}.json`,
    state.yearlyData
  );
}
...
export function reorderLabels(orderedProjectIds) {
  // reorderProjects
...
export function updateLabelName(labelId, newName) {
  ...
  if (label && label.name !== newName) {
    ...
    eventBus.dispatch("dataChanged", {
      source: "updateLabelName",
      payload: { labelId, newName },
    });
    dirtyFileService.markFileAsDirty(
      `${state.view.currentDisplayYear}/${state.view.currentDisplayYear}.json`,
      state.yearlyData
...
export function deleteLabelAndAssociatedEvents(labelId) {
  ...
  if (state.yearlyData.labels) {
    ...
    state.yearlyData.labels = state.yearlyData.labels.filter(
  ...
  if (state.yearlyData.events) {
    ...
    state.yearlyData.events = state.yearlyData.events.filter(
      (event) => event.labelId !== labelId
    );
  ...
  if (changed) {
    ...
    console.log(
      `[DataManager] Label and associated events deleted for labelId: ${labelId}`
    );
  }
}
...
export function deleteBacklogTodo(todoId) {
  ...
  state.yearlyData.backlogTodos = state.yearlyData.backlogTodos.filter(
    (todo) => todo.id !== todoId
  );
...
export function addTodoForDate(dateStr, todoText, todoDetails = {}) {
  ...
  const monthDataObject = JSON.parse(
    JSON.stringify(
      getRawDailyDataForMonth(yearMonth) || {
        yearMonth,
        routines: [],
        colorPalette: [],
        dailyData: {},
...
export function updateTodoPropertyForDate(
  dateStr,
  todoId,
  propertyName,
  newValue
...
export function moveBacklogTodoToCalendar(todoId, targetDate) {
  ...
  const [todoItemFromBacklog] = state.yearlyData.backlogTodos.splice(
    todoToMoveIndex,
    1
  );
  eventBus.dispatch("dataChanged", {
    source: "moveBacklogTodoToCalendar_removedFromBacklog",
    payload: {
      /* updatedBacklog: state.yearlyData.backlogTodos - 직접 전달보다 이벤트를 통해 UI가 getState() 하도록 유도 */
    },
  });
  dirtyFileService.markFileAsDirty(
    `${state.view.currentDisplayYear}/${state.view.currentDisplayYear}.json`,
  ...
  const newCalendarTodo = {
    ...
    importance:
      todoItemFromBacklog.priority !== undefined
        ? todoItemFromBacklog.priority
        : 0,
    time: 0,
...
export function moveWeeklyTodoToBacklog(todoId) {
  // 주간 캘린더에서 백로그로 할 일을 이동하는 함수
  if (!state.yearlyData || !state.yearlyData.backlogTodos) return;

  // 모든 월의 데이터에서 해당 todoId를 찾아서 제거
  let foundTodo = null;
  let foundDate = null;
  ...
  if (foundTodo) {
    // 백로그에 새 할 일 추가 (우선순위 0으로 설정)
    const newBacklogTodo = {
      id: generateId("backlog_"),
      text: foundTodo.text,
      priority: foundTodo.importance !== undefined ? foundTodo.importance : 0,
      color: foundTodo.color || "#6c757d",
      completed: false,
    };

    state.yearlyData.backlogTodos.push(newBacklogTodo);
    dirtyFileService.markFileAsDirty(
      `${state.view.currentDisplayYear}/${state.view.currentDisplayYear}.json`,
      state.yearlyData
    );

    console.log(`Moved todo "${foundTodo.text}" from ${foundDate} to backlog`);
    eventBus.dispatch("dataChanged", {
      source: "moveWeeklyTodoToBacklog",
      payload: {
        movedTodo: newBacklogTodo,
        fromDate: foundDate,
      },
    });
  } else {
    console.warn(`Todo with ID ${todoId} not found in any calendar date`);
  }
}
...
export function moveYearlyTodoToAnotherDate(todoId, targetDate) {
  // 연간 캘린더 내에서 할 일을 다른 날짜로 이동하는 함수
  let foundTodo = null;
  let foundDate = null;
  ...
  if (foundTodo && targetDate) {
    ...
    let monthData = getRawDailyDataForMonth(targetYearMonth);
    ...
    // 기존 할 일을 그대로 추가 (ID 유지)
    dayData.todos.push(foundTodo);
    updateDailyData(targetYearMonth, monthData);

    console.log(
      `Moved yearly todo "${foundTodo.text}" from ${foundDate} to ${targetDate}`
    );
    eventBus.dispatch("dataChanged", {
      source: "moveYearlyTodoToAnotherDate",
      payload: {
        movedTodo: foundTodo,
        fromDate: foundDate,
        toDate: targetDate,
...
export function getSpecificYearDataForSave(yearToSave) {
  ...
  let yearDataContent =
    state.yearlyData && state.yearlyData.year === year
      ? state.yearlyData
      : dirtyFileService.getDirtyFileData(yearlyIdentifier);

  // 만약 위에서 데이터를 못 찾고, 강제로라도 해당 연도 파일을 만들어야 한다면, 기본 구조라도 생성.
  if (!yearDataContent && state.view.currentDisplayYear === year) {
    // 현재 작업중인 연도라면 메모리 상태를 사용
    yearDataContent = state.yearlyData;
  }
  if (!yearDataContent) {
    // 그래도 없다면 최소 구조 (주로 fetch 실패 시)
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    let monthDataObject =
      dirtyFileService.getDirtyFileData(dailyFileIdentifier);
...
export function markYearlyDataAsDirty() {
  if (state.yearlyData) {
    dirtyFileService.markFileAsDirty(
      `${state.yearlyData.year}/${state.yearlyData.year}.json`,
      state.yearlyData
    );
  }
}
...
export function deleteProjectPermanently(projectId) {
  ...
  if (state.yearlyData.projects) {
    ...
    state.yearlyData.projects = state.yearlyData.projects.filter(
      (p) => p.id !== projectId
    );
