...
export function renderWeeklyCalendar(weekStartDate) {
  if (!weeklyCalendarArea) {
    console.error(
      "[renderWeeklyCalendar] weeklyCalendarArea is not initialized. Call initWeeklyCalendar first."
    );
    return;
  }
...
function renderWeeklyDayCellContent(dateStr, cellContentContainer) {
  ...
  // Show daily todos: regular todos always, injected P1 only when completed
  const todosForDay = (data.getTodosForDate(dateStr) || []).filter((t) => {
    if (t.injectedFromBacklogP1) {
      return t.completed === true; // injected P1 only when completed
    }
    return true; // regular todos always show
  });
  const itemsToRender = [];
...
function handleCellDrop(e) {
  ...
  if (e.dataTransfer.types.includes("application/x-backlog-source")) {
    ...
    if (draggedTodoOriginalDate === targetDate) {
      ...
      const todoToMove = data
        .getTodosForDate(draggedTodoOriginalDate)
        .find((t) => t.id === droppedTodoId);
      if (todoToMove) {
...
function reorderTodoInCell(e, dayCell, droppedTodoId) {
  ...
  const todoElements = Array.from(
    contentContainer.querySelectorAll(".mv-weekly-todo-item")
  ); // 접두사 적용
...
function handleWeeklyCellDoubleClick(e) {
  ...
  if (dateStr) {
    console.log(
      `[WeeklyCalendar] Cell double-clicked: ${dateStr}. Navigating to daily view...`
    );
