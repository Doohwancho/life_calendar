...
// --- 기본값 정의 ---
const DEFAULT_TIME_BLOCK_COLOR = "#ffffff";
const DEFAULT_GOAL_BLOCK_COLOR = "#ffffff";
const DEFAULT_BLOCK_TEXT = "";
// const DEFAULT_PREVIOUS_COLOR = null;

// --- Callbacks & Refs ---
let getSelectedColorCallback = () => "#ffffff";
...
function storeCurrentColorsForGrid(internalGridKey) {
  ...
  const colorStoreMap =
    internalGridKey === "timeGrid" ? timeGridStartColors : goalGridStartColors;
  colorStoreMap.clear();

  const actualDomId =
    internalGridKey === "timeGrid" ? timeGridActualDomId : goalGridActualDomId;
  ...
  document.querySelectorAll(`#${actualDomId} .dv-grid-cell`).forEach((cell) => {
    ...
    const colorToStore = currentCellData ? currentCellData.color : "#ffffff";
    colorStoreMap.set(key, colorToStore);
  });
}

function internalSaveBlock(
  blockKey,
  text,
  cellElement,
  internalGridKey,
  newColor,
  previousColorForInteractionStart
) {
  const dataStore = internalGridKey === "timeGrid" ? blockData : goalBlockData;
  const defaultColor =
    internalGridKey === "timeGrid"
      ? DEFAULT_TIME_BLOCK_COLOR
      : DEFAULT_GOAL_BLOCK_COLOR;
  ...
  dataStore[blockKey].color = normalizeColorToHex(newColor);
...
function handleCellClickInternal(event, internalGridKey) {
  ...
  if (!cellData) {
    console.warn(
      `[TIMELINES] Data for ${blockKey} not pre-initialized. Using defaults.`
    );
    const defaultColor =
      internalGridKey === "timeGrid"
        ? DEFAULT_TIME_BLOCK_COLOR
  ...
  saveBtn.className = "dv-modal-button dv-save-text-button";
  saveBtn.textContent = "Save";
  const cancelBtn = document.createElement("button");
  cancelBtn.className = "dv-modal-button dv-cancel-text-button";
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const originalColorAtInteractionStart =
      colorStoreMap.get(blockKey) || cellData.color;
    const effectivePreviousColor =
      internalGridKey === "timeGrid" ? originalColorAtInteractionStart : null;
    internalSaveBlock(
      blockKey,
      textarea.value,
      cell,
  ...
  textarea.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      internalSaveBlock(
        ...
        dataStore[blockKey]?.previousColor
      );
      closeModal();
    }
  });
  ...
  internalSaveBlock(
    ...
    cell,
    internalGridKey,
    selectedColorFromPicker,
    colorBeforeThisClick
  );
...
function handleDragPaintInternal(
  draggedStartCell,
  currentHoverCell,
  internalGridKey
) {
  ...
  const dataStore = internalGridKey === "timeGrid" ? blockData : goalBlockData;
  const startColorsMap =
    internalGridKey === "timeGrid" ? timeGridStartColors : goalGridStartColors;

  const minHour = Math.min(
    parseInt(draggedStartCell.dataset.hour),
    parseInt(currentHoverCell.dataset.hour)
  );
  ...
  const minBlock = Math.min(
    parseInt(draggedStartCell.dataset.block),
    parseInt(currentHoverCell.dataset.block)
  );
  ...
  for (let h = minHour; h <= maxHour; h++) {
    for (let b = minBlock; b <= maxBlock; b++) {
      ...
      const cellElement = document.querySelector(
        `#${actualDomId} .dv-grid-cell[data-hour="${h}"][data-block="${b}"]`
      );
      if (cellElement) {
        const colorBeforeDragStarted = startColorsMap.get(cellKey);
        const currentText = dataStore[cellKey]?.text || "";
        const effectivePreviousColor =
          internalGridKey === "timeGrid" ? colorBeforeDragStarted : null;
        internalSaveBlock(
          cellKey,
          currentText,
          cellElement,
...
export function initTimelines(timeGridDomId, goalGridDomId, callbacks) {
  ...
  if (callbacks) {
    getSelectedColorCallback =
      callbacks.getSelectedColor || getSelectedColorCallback;
    ...
    onTimeGridDataChangeCallback =
      callbacks.onTimeGridDataChange || onTimeGridDataChangeCallback;
  }
...
function buildGridStructure(gridDomId, internalGridKey) {
  ...
  if (!gridElement) {
    console.error(
      `[TIMELINES] CRITICAL: Grid element with ID "${gridDomId}" NOT FOUND.`
    );
    return;
  }
  ...
  for (let hour = 0; hour < 24; hour++) {
    // This hour is the grid's internal hour (0 for 06:00, 1 for 07:00, etc.)
      ...
      cell.addEventListener("drop", (e) => {
        ...
        if (jsonData) {
          try {
            ...
            console.error(
              "[TIMELINES] Error parsing dropped data:",
              err,
              jsonData
            );
          }
  ...
  for (let hour = 0; hour < 24; hour++) {
    for (let blockIdx = 0; blockIdx < 6; blockIdx++) {
      ...
      const cell = gridElement.querySelector(
        `.dv-grid-cell[data-hour="${hour}"][data-block="${blockIdx}"]`
      );
      if (!cell) continue;
      ...
      if (!dataStore[blockKey]) {
        dataStore[blockKey] = {
          text: DEFAULT_BLOCK_TEXT,
          color: normalizeColorToHex(defaultColor),
        };
        // if (internalGridKey === 'timeGrid') {
        //     dataStore[blockKey].previousColor = DEFAULT_PREVIOUS_COLOR;
        // }
      ...
      // 저장된 색상을 hex로 정규화
      const normalizedColor = normalizeColorToHex(cellData.color);
      cellData.color = normalizedColor;
      cell.style.backgroundColor = normalizedColor;
      cell.textContent = cellData.text;
      cell.style.color = isDarkColorCallback(normalizedColor)
        ? "white"
        : "black";
...
function renderScheduledTasksOnGrid(gridKey) {
  const actualDomId =
    gridKey === "timeGrid" ? timeGridActualDomId : goalGridActualDomId;
  const gridElement = document.getElementById(actualDomId);
  if (!gridElement) return;

  let overlayContainer = gridElement.querySelector(
  ...
  const tasksForThisGrid = scheduledTasks.filter(
    (task) => task.gridKey === gridKey
  );
  ...
  tasksForThisGrid.forEach((task) => {
    ...
    while (blocksProcessed < totalBlocksToProcess && currentHour < 24) {
      const blocksInCurrentRow = Math.min(
        6 - currentBlockInHour,
        totalBlocksToProcess - blocksProcessed
      );
      const firstCellInSegment = gridElement.querySelector(
        `.dv-grid-cell[data-hour="${currentHour}"][data-block="${currentBlockInHour}"]`
      );
      ...
      segmentOverlay.style.borderColor =
        task.color || DEFAULT_SCHEDULED_TASK_BORDER_COLOR;
...
function handleResizeMove(e) {
  ...
  if (resizingTaskInfo.handle === "right") {
    ...
    console.log(
      `Start Index: ${startCellIndex}, Target Index: ${targetCellIndex}, New Block Count: ${newNumBlocks}`
    );
  } else if (resizingTaskInfo.handle === "left") {
    const endCellIndex =
      resizingTaskInfo.initialStartCellIndex +
      resizingTaskInfo.initialNumBlocks -
      1;
    ...
    console.log(
      `End Index (fixed): ${endCellIndex}, New Start Index: ${newStartCellIndex}, New Block Count: ${newNumBlocks}`
    );
  }
...
function isBlockDefault(block, isGoalBlock = false) {
  ...
  const defaultColor = isGoalBlock
    ? DEFAULT_GOAL_BLOCK_COLOR
    : DEFAULT_TIME_BLOCK_COLOR;
  const textIsDefault =
    block.text === DEFAULT_BLOCK_TEXT || typeof block.text === "undefined";
  const colorIsDefault =
    normalizeColorToHex(block.color) === normalizeColorToHex(defaultColor) ||
    typeof block.color === "undefined";
  if (isGoalBlock) {
...
export function getTimelineBlockData() {
  ...
  for (const key in blockData) {
    if (!isBlockDefault(blockData[key], false)) {
      filteredBlockData[key] = {
        ...blockData[key],
        color: normalizeColorToHex(blockData[key].color),
      };
    }
  }
...
export function getTimelineGoalData() {
  ...
  for (const key in goalBlockData) {
    if (!isBlockDefault(goalBlockData[key], true)) {
      filteredGoalData[key] = {
        ...goalBlockData[key],
        color: normalizeColorToHex(goalBlockData[key].color),
      };
    }
  }
  ...
  if (newData) {
    for (const key in newData) {
      blockData[key] = {
        ...
        color: normalizeColorToHex(newData[key].color),
      };
    }
  }
  ...
  if (newGoalData) {
    for (const key in newGoalData) {
      goalBlockData[key] = {
        ...
        color: normalizeColorToHex(newGoalData[key].color),
      };
    }
  }
...
export function applySelectedColorToCurrentlyEditingCell() {
  if (currentlyEditingCellInternal && activeGridIdInternal) {
    ...
    const dataStore =
      activeGridIdInternal === "timeGrid" ? blockData : goalBlockData;
    const colorStoreMap =
      activeGridIdInternal === "timeGrid"
        ? timeGridStartColors
        : goalGridStartColors;
    const colorBeforeInteraction =
      colorStoreMap.get(blockKey) ||
      currentlyEditingCellInternal.style.backgroundColor;
    const currentText = dataStore[blockKey]?.text || "";
    internalSaveBlock(
      blockKey,
      currentText,
      currentlyEditingCellInternal,
      activeGridIdInternal,
      selectedColor,
      colorBeforeInteraction
    );
    ...
    if (modalContent && textarea) {
      ...
      textarea.style.color = isDarkColorCallback(selectedColor)
        ? "white"
        : "black";
    }
  }
...
function addScheduledTaskToTimeline(
  todoData,
  startHourStr,
  startBlockStr,
  gridKey
...
export function setScheduledTasksDataAndRender(
  newDataArray,
  gridKeysToRender = ["timeGrid", "goalTimeGrid"]
) {
  ...
  const validGridKeys = [];
  if (
    gridKeysToRender.includes("timeGrid") &&
    document.getElementById(timeGridActualDomId)
  ) {
    validGridKeys.push("timeGrid");
  }
  if (
    gridKeysToRender.includes("goalTimeGrid") &&
    document.getElementById(goalGridActualDomId)
  ) {
...
export function updatePassedTimeVisuals(
  gridDomIdToUpdate,
  isTimelineEffectivelyToday,
  dateOfTimelineString
) {
  ...
  if (
    currentHourForGridLogic < ACTUAL_DAY_STARTS_AT_HOUR &&
    timelineDateObject.getTime() < todayObject.getTime()
  ) {
  ...
  const gridHourIndex =
    (currentHourForGridLogic - ACTUAL_DAY_STARTS_AT_HOUR + 24) % 24;
  ...
  const totalPassedBlocksInGridTimeline =
    gridHourIndex * BLOCKS_PER_HOUR + gridBlockIndexInHour;

  allDataCells.forEach((cell) => {
    const cellDataHour = parseInt(cell.dataset.hour, 10); // 셀의 그리드 시간 인덱스 (0은 06시)
    const cellDataBlock = parseInt(cell.dataset.block, 10);
    const cellOverallBlockIndexInGrid =
      cellDataHour * BLOCKS_PER_HOUR + cellDataBlock;
    ...
    if (cellOverallBlockIndexInGrid < totalPassedBlocksInGridTimeline) {
      ...
      if (typeof isDarkColorCallback === "function") {
        // isDarkColorCallback은 timelines.js 스코프에 있어야 함
        const cellBgColor =
          cell.style.backgroundColor ||
          window.getComputedStyle(cell).backgroundColor;
        const normalizedBgColor = normalizeColorToHex(cellBgColor);
        cell.classList.toggle(
          "dv-dark-bg",
          isDarkColorCallback(normalizedBgColor)
        );
        cell.classList.toggle(
          "dv-light-bg",
          !isDarkColorCallback(normalizedBgColor)
    ...
    // 현재 시간 블록에 빨간 줄 표시
      ...
      cellDataHour === gridHourIndex &&
      cellDataBlock === gridBlockIndexInHour
    ) {
      cell.classList.add("dv-current-time-block");
      const percentageThroughBlock =
        ((currentMinute % MINUTES_PER_BLOCK) / MINUTES_PER_BLOCK) * 100;
