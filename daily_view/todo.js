...
function getDragAfterElement(containerUL, y) {
  ...
  const draggableElements = [
    ...containerUL.querySelectorAll(".dv-todo-item:not(.dragging-todo)"),
  ];
...
function createTodoElement(item, index) {
  ...
  li.addEventListener("dragstart", (e) => {
    ...
    try {
      ...
      console.log(
        `[TODO.JS] Data set for timeline: ${dataString}. effectAllowed: ${e.dataTransfer.effectAllowed}`
      );
    } catch (err) {
  ...
  li.addEventListener("dragend", (e) => {
    ...
    if (todoULElement) {
      // << 수정됨: dv- 접두사 추가
      todoULElement
        .querySelectorAll(".dv-todo-item.dv-drag-over-todo")
        .forEach((el) => {
  ...
  checkbox.addEventListener("change", () => {
    ...
    // If this was injected from backlog P1 and now completed, remove from backlog
    if (
      item.completed &&
      item.injectedFromBacklogP1 &&
      item.originalBacklogId
    ) {
      try {
        // Use dynamic import to avoid circular dependencies
        import("../js/dataManager.js")
          .then((dm) => {
            if (typeof dm.deleteBacklogTodo === "function") {
              dm.deleteBacklogTodo(item.originalBacklogId);
            }
          })
          .catch(() => {});
      } catch (e) {
...
function renderTodoListInternal() {
  ...
  if (
    focusedTodoId &&
    focusedElement?.classList.contains("dv-todo-text-input")
  ) {
    const newFocusedElement = todoULElement.querySelector(
      `.dv-todo-item[data-id="${focusedTodoId}"] .dv-todo-text-input`
    );
...
export function initTodoApp(
  ...
  if (!container) {
    console.error(`TODO app container "${containerSelector}" not found.`);
    return;
  }
  ...
  todoULElement.addEventListener("dragover", (e) => {
    ...
    const currentOverElement =
      todoULElement.querySelector(".dv-drag-over-todo");
  ...
  todoULElement.addEventListener("dragleave", (e) => {
    if (
      e.target === todoULElement &&
      !todoULElement.contains(e.relatedTarget)
    ) {
      // << 수정됨: dv- 접두사 추가
      const currentOverElement =
        todoULElement.querySelector(".dv-drag-over-todo");
  ...
  addBtnContainerElement = todoListContainerElement.querySelector(
...
export function setTodoDataAndRender(newTodoData) {
  todos = Array.isArray(newTodoData)
    ? newTodoData.map((todo) => ({ ...todo, id: todo.id || generateId() }))
    : [];
  renderTodoListInternal();
}
